...
def build_generator():
    ...
    H = Dense(14 * 14 * 128, init='glorot_normal')(inLayer)
    H = BatchNormalization(mode=2)(H)
    H = Activation('relu')(H)
    H = Reshape([14, 14, 128])(H)
    H = UpSampling2D(size=(2, 2), dim_ordering='tf')(H)
    H = Convolution2D(nb_row=3,
                      nb_col=3,
                      nb_filter=128,
                      border_mode='same',
                      dim_ordering='tf',
    ...
    H = BatchNormalization(mode=2)(H)
    H = Activation('relu')(H)
    H = Convolution2D(nb_filter=1,
                      nb_row=1,
                      nb_col=1,
                      dim_ordering='tf',
                      border_mode='same', init='glorot_uniform')(H)
...
def train_gan_on_batch(gModel, dModel, ganModel, inReal):
    dLoss, dTime = train_disc_on_batch(GEN, DSC, GAN, inReal)
    gLoss, gTime = train_gen_on_batch(GEN, DSC, GAN, inReal.shape[0])
